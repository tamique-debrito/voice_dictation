"""HTTP server for offline annotation of a session directory.
...
Routes
------
``GET  /``                            → redirect to ``/annotate``
``GET  /annotate``                    → annotation HTML page
``GET  /annotation/chunks``           → per-chunk view (segments split on
                                         word midpoints at chunk boundaries)
``GET  /annotation/audio/<idx>``      → ``audio/chunk_NNNN.wav`` with
                                         ``Range`` support
``GET  /annotation/state``            → resolved last-write-wins state
``PUT  /annotation/<idx>/<seg_id>``   → append a segment annotation
``PUT  /annotation/<idx>``            → append a chunk-level annotation
                                         (reject / unreject)
"""
...
class AnnotatorServer:
    def __init__(
        ...
        *,
        ...
        self._port = port
        self._store = AnnotationStore(self._session_dir)
        self._chunks: list[ChunkView] = load_chunks(self._session_dir)
    ...
    def start(self) -> None:
        ...
        class _Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self) -> None:
                ...
                if p == "/" or p == "/annotate":
                    ...
                    return
                if p == "/annotation/chunks":
                    self._send_json(outer._chunks_payload())
                    return
                if p == "/annotation/state":
                    self._send_json(outer._store.resolved_state())
                    return
                m = re.match(r"^/annotation/audio/(\d+)$", p)
                if m:
                    outer._serve_audio(self, int(m.group(1)))
                    return
                self.send_error(404)
            ...
            def do_PUT(self) -> None:
                p = unquote(self.path)
                # /annotation/<idx>/<seg_id>
                m = re.match(r"^/annotation/(\d+)/(.+)$", p)
                if m:
                    chunk_idx = int(m.group(1))
                    seg_id = m.group(2)
                    body = self._read_body()
                    try:
                        ann = outer._store.append_segment(
                            chunk_idx=chunk_idx,
                            segment_id=seg_id,
                            text=body.get("text", ""),
        ...
        self._thread.start()
        logger.info(
            "AnnotatorServer started on http://127.0.0.1:%d/  session=%s "
            "chunks=%d", self.actual_port, self._session_dir, len(self._chunks),
        )
    ...
    def _chunks_payload(self) -> dict:
        return {
            "session_dir": str(self._session_dir),
            "chunks": [c.to_dict() for c in self._chunks],
        }

    def _serve_audio(self, handler: BaseHTTPRequestHandler, idx: int) -> None:
        if idx < 0 or idx >= len(self._chunks):
            handler.send_error(404)
            return
        wav = self._audio_dir / self._chunks[idx].file
        if not wav.exists():
            handler.send_error(404)
            return
...
# ---------------------------------------------------------------------------
# HTML (kept short; the real annotation page is the next file in this PR.)
# ---------------------------------------------------------------------------
...
#picker { width: 200px; border-right: 1px solid #333; overflow-y: auto; flex-shrink: 0; }
#picker .row {
...
<div id="app">
  ...
  <div id="main">
    ...
    <div id="chunk-actions">
      ...
      <span>keys: ↑/↓ pick chunk · space play/pause · click ts to seek · edits auto-save on blur · r reject</span>
    </div>
  </div>
</div>
...
# The JS lives in a separate function so the server can hot-edit it without
# touching the HTML scaffold. (We don't actually serve /static/annotate.js;
# the loader inlines the script via the BOOT placeholder.)
...
async function loadAll() {
  const [chunks, state_] = await Promise.all([
    fetch("/annotation/chunks").then(r=>r.json()),
    fetch("/annotation/state").then(r=>r.json()),
  ]);
...
function selectChunk(idx) {
  ...
  if (!c) return;
  $("#player").src = `/annotation/audio/${idx}`;
...
function autoSize(ta) {
  // Grow textarea to fit content; never shrink below one line.
  // scrollHeight is only reliable when the element is in the DOM.
  ta.style.height = "auto";
  const h = Math.max(ta.scrollHeight, 26);
  ta.style.height = h + "px";
}

// Word-level diff (LCS-based) — returns array of ["=", str] | ["-", str] | ["+", str].
function wordDiff(a, b) {
  const ta = a.split(/(\s+)/);
  const tb = b.split(/(\s+)/);
  const m = ta.length, n = tb.length;
  // LCS DP from bottom-right.
  const dp = Array(m + 1);
  ...
  // Coalesce consecutive same-op tokens to reduce DOM nodes.
  const out = [];
...
function renderSegments() {
  ...
  for (const s of c.segments) {
    ...
    // [ts] [accent] [source/diff] [edit cell] [status-dot]
    ...
    el.appendChild(div);
    // autoSize only works once the element is attached to the DOM,
    // because scrollHeight reads layout. Run it after appendChild.
    autoSize(ta);
  }
}
...
async function maybeSaveOnBlur(s, div, ta) {
  ...
  // No-op if value matches source AND there's no prior annotation, or
  // matches prior annotation. Don't spam writes.
...
async function saveSegment(s, status, div, textOverride) {
  ...
  const r = await fetch(
    `/annotation/${c.chunk_idx}/${encodeURIComponent(s.segment_id)}`,
  ...
  // Don't re-render everything; just update the picker rail status.
  renderPicker();
  toast("saved");
}
...
async function toggleReject() {
  ...
  const r = await fetch(`/annotation/${c.chunk_idx}`,
    {method: "PUT", headers: {"Content-Type": "application/json"},
...
# Inline the JS into the HTML at module-import time.
_INDEX_HTML = _INDEX_HTML.replace("__BOOT__", _ANNOTATE_JS).replace(
